    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 flex-1 w-full">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <p className="text-blue-600 font-bold text-sm uppercase tracking-widest mb-3">Pricing</p>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-3xl p-8 flex flex-col ${
                plan.highlight
                  ? 'bg-slate-950 text-white shadow-2xl shadow-blue-600/20 ring-1 ring-blue-500/50 lg:scale-105'
                  : 'card'
              }`}
            >
              {plan.highlight && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 badge bg-gradient-to-r from-blue-600 to-sky-600 text-white shadow-lg px-4">
                  Most popular
                </span>
              )}
              ...
              <p className={`text-sm ${plan.highlight ? 'text-blue-300' : 'text-blue-600'} font-medium mb-3`}>{plan.sub}</p>
              ...
              <ul className="space-y-3.5 mb-8 flex-1">
                {plan.features.map((f) => (
                  <li key={f} className={`flex items-start gap-3 text-sm ${plan.highlight ? 'text-slate-300' : 'text-slate-600'}`}>
                    <svg className={`w-5 h-5 flex-shrink-0 ${plan.highlight ? 'text-blue-400' : 'text-blue-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              ...
              <Link
                ...
                className={plan.highlight
                  ? 'inline-flex items-center justify-center bg-white text-slate-900 font-bold px-6 py-3.5 rounded-xl hover:bg-blue-50 active:scale-[0.98] transition-all'
