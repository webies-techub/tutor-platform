...
export default function TutorProfile() {
  ...
  useEffect(() => {
    api.get('/tutors/profile')
      .then(({ data }) => {
        setProfile(data);
        setForm({ bio: data.bio || '', subjects: data.subjects || '', hourly_rate: data.hourly_rate || '' });
      })
      .catch(() => {})
  ...
  const avatarSrc = preview || (profile?.avatar_path ? `http://localhost:3001/${profile.avatar_path}` : null);
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleSubmit} className="card p-8 sm:p-10 space-y-7">
          ...
          <div className="flex items-center gap-6">
            {avatarSrc ? (
              <img src={avatarSrc} alt="Avatar" className="w-20 h-20 rounded-3xl object-cover ring-4 ring-violet-100" />
            ) : (
              <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-display font-bold">
                ?
              </div>
            )}
            <div>
