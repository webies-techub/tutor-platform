      ...
      <aside className="hidden lg:flex w-64 flex-col bg-white border-r border-slate-200 fixed inset-y-0">
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 px-2">
            <span className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white font-bold">
          ...
          <div className="flex gap-1">
            {items.map((item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `p-2.5 rounded-lg ${isActive ? 'bg-violet-50 text-violet-700' : 'text-slate-400'}`
                }
              >
                <Icon d={item.icon} />
              </NavLink>
